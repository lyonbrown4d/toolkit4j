 ...
 * 集合运算工具类。
 *
 * <p>职责限定为：多集合之间的组合与关系运算， 如拼接、并集、交集、差集、对称差集、子集判断等。
 *
 * <p>设计约定：
 *
 * <ul>
 *   <li>对 {@code null} 集合作为空集合处理
 *   <li>返回结果优先保持遍历顺序
 *   <li>返回类型统一为 {@link List}，便于调用方直接消费
 * </ul>
 */
@UtilityClass
public class CollectionOperations {
  ...
  public <T> List<T> concat(Collection<? extends T>... collections) {
    return streamOf(collections).toList();
  }
  ...
  public <T> List<T> union(Collection<? extends T>... collections) {
    return streamOf(collections).distinct().toList();
  }

  /**
   * 多集合交集。
   *
   * <p>语义： 结果中只保留所有集合都包含的元素，且结果去重。 返回顺序以第一个非空集合中的遍历顺序为准。
   *
   * @param collections 多个集合
   * @param <T> 元素类型
  ...
  public <T> List<T> intersection(Collection<? extends T>... collections) {
    ...
    val nonNullCollections = Stream.of(collections).filter(Objects::nonNull).toList();
    ...
    val remainingSets = nonNullCollections.stream().skip(1).map(LinkedHashSet::new).toList();
   ...
   * @return 差集结果
   */
  public <T> List<T> difference(Collection<? extends T> left, Collection<? extends T> right) {
    if (left == null || left.isEmpty()) {
      return Collections.emptyList();
    }

    val rightSet = right == null ? emptySet() : new LinkedHashSet<>(right);
  ...
  public <T> List<T> symmetricDifference(
    ...
    return Stream.concat(difference(left, right).stream(), difference(right, left).stream())
        .distinct()
        .toList();
  }
   ...
   *
   * <ul>
   *   <li>{@code null} subset 视为空集
   *   <li>{@code null} superset 视为空集
   * </ul>
   *
   * @param subset 候选子集
   * @param superset 候选父集
   ...
   */
  public boolean isSubset(Collection<?> subset, Collection<?> superset) {
   ...
   *
   * <ul>
   *   <li>subset 是 superset 的子集
   *   <li>subset 与 superset 不相等
   * </ul>
   *
   * @param subset 候选子集
   * @param superset 候选父集
   ...
   */
  public boolean isProperSubset(Collection<?> subset, Collection<?> superset) {
    if (!isSubset(subset, superset)) {
      return false;
    }

    val subsetSet = subset == null ? emptySet() : new LinkedHashSet<>(subset);
    val supersetSet = superset == null ? emptySet() : new LinkedHashSet<>(superset);
